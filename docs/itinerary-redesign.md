...
The itinerary always spans the whole trip. Every date appears once. Contiguous destination groups have centered numbered headings aligned with the overnight stay sections. Pale destination colors alternate diagonal stripes, dots, stars and grids. On transfer days, hourly backgrounds follow the previous destination before travel, turn neutral during travel, and adopt the next destination after arrival. Dropping an activity on a shared date preserves its destination.

The calendar sits on the left with the idea pool on the right. Synchronized slim horizontal scrollbars sit above and below the table; inner vertical scrollbars are hidden. Dragging empty calendar space pans in both directions, while notes retain their activity drag behavior and buttons remain clickable. Wheel, touch and keyboard scrolling remain available. Accommodation occupies one compact row above the clearly separated date headings; adjacent cells merge until the stay or set of stays changes. Confirmed local check-in and check-out dates control coverage; checkout does not count as another night. Missing dates use the destination range and are marked for confirmation. Unplaced stays and transport remain accessible under Dates to review.
...
- Latest calendar refinement: 66 web tests, lint, typecheck and production web build. Browser interaction and visual QA remain unperformed.
