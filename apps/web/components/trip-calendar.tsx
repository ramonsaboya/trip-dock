...
import { calendarColumns, stayCoversDay, transportPlacement, tripRoutes } from '../lib/trip-calendar';
...
export function TripCalendar({ trip, onChanged, onActivity, onStay, onTransport, onDestination, onRemove }: {
  ...
  useEffect(() => {
    ...
    const morning = board?.querySelector<HTMLElement>('[data-hour="08:00"]');
  ...
  const bands: Array<{ key: string; label: string; color: number; span: number; stop?: TripStop }> = [];
  visible.forEach((column) => {
    const key = column.destination?.id ?? 'unassigned';
    const last = bands.at(-1);
    if (last?.key === key) last.span += 1;
    else bands.push({ key, label: column.destination?.name ?? 'Dates open', color: column.color, span: 1, stop: column.destination });
  });
  ...
  function transportNote(leg: TransportLeg) {
    ...
    return <article className="journey-note" key={leg.id}>
      <button type="button" className="record-main" onClick={() => onTransport(leg)}><span className="journey-note-mode">↗ {leg.mode}</span><strong>{leg.title}</strong><span>{from} → {to}</span><small>{placement.suggested ? 'Time to confirm · date from destination' : `${formatDateTime(leg.departureTime, leg.timezone)} → ${formatDateTime(leg.arrivalTime, leg.timezone)}`}</small></button>
  ...
  return <section className="unified-trip-calendar" aria-label="Trip calendar" aria-busy={busy}>
    ...
    <div className="trip-calendar-workspace">
      ...
      <div className="trip-calendar-scroll" ref={viewport} tabIndex={0} role="region" aria-label="Itinerary by date">
        <table className="trip-calendar-table"><colgroup><col style={{ width: 62 }} />{visible.map((column) => <col key={column.day} />)}</colgroup>
          <thead ref={header}>
            <tr className="destination-band"><th scope="row">Places</th>{bands.map((band, index) => <th key={`${band.key}-${index}`} colSpan={band.span} className={`destination-tint-${band.color}`} scope="colgroup">{band.stop ? <button type="button" onClick={() => onDestination(band.stop!)}><span>{String(stops.findIndex((stop) => stop.id === band.stop!.id) + 1).padStart(2, '0')}</span> {band.label}</button> : band.label}</th>)}</tr>
            <tr className="calendar-stay-row"><th scope="row">Stay</th>{visible.map((column) => <td key={column.day} className={`destination-tint-${column.color}`}><div className="calendar-stay-items">{trip.stays.filter((stay) => stayCoversDay(stay, column.day, stops)).map(stayButton)}</div>{column.destination ? <button type="button" className="calendar-add-stay" aria-label={`Add stay in ${column.destination.name} for ${dateLabel(column.day)}`} onClick={() => onStay(undefined, column.destination!.id)}>+ Stay</button> : null}</td>)}</tr>
            <tr className="calendar-date-row"><th scope="row">Date</th>{visible.map((column) => <th scope="col" key={column.day} className={`destination-tint-${column.color}`}>{dateLabel(column.day)}{column.destinations.length > 1 ? <small>{column.destinations.map((stop) => stop.name).join(' → ')}</small> : null}</th>)}</tr>
            <tr className="travel-planning-row"><th scope="row">Travel</th>{visible.map((column) => <td key={column.day} className={`destination-tint-${column.color}`}><div className="journey-options">{trip.transportLegs.filter((leg) => { const place = transportPlacement(leg, stops); return place.day === column.day && !place.hour; }).map(transportNote)}</div>{routes.filter((route) => route.day === column.day && !trip.transportLegs.some((leg) => leg.fromStopId === route.fromStopId && leg.toStopId === route.toStopId)).map((route) => <button key={`${route.fromStopId}-${route.toStopId}`} type="button" className="plan-journey" onClick={() => onTransport(undefined, route.fromStopId, route.toStopId)}>↗ {route.label}<small>+ Plan transport</small></button>)}</td>)}</tr>
          </thead>
          <tbody>

            {calendarHours.map((hour) => <tr key={hour}><th scope="row" data-hour={hour}>{hour}</th>{visible.map((column) => <td key={column.day} className={`trip-calendar-hour destination-tint-${column.color} ${dropTarget === `${column.day}-${hour}` ? 'drop-active' : ''}`} onDragOver={(event) => dragOver(event, `${column.day}-${hour}`)} onDrop={(event) => void drop(event, column.destination?.id, column.day, hour)} aria-label={`${dateLabel(column.day)} at ${hour}${column.destination ? ` in ${column.destination.name}` : ''}`}>
              <div className="journey-options">{trip.transportLegs.filter((leg) => { const place = transportPlacement(leg, stops); return place.day === column.day && place.hour === hour; }).map(transportNote)}</div>
    ...
    <p className="planner-hint">Times use each record’s timezone. On shared travel days, dropping an activity assigns it to the arriving destination. Click a note to change its destination or exact time.</p>
