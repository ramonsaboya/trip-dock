...
if (!pnpm) throw new Error('Start this preview with pnpm dev:packing.');

const env = {
  ...process.env,
  DATABASE_URL: 'postgresql://tripdock:tripdock@127.0.0.1:55434/tripdock_packing',
  API_PORT: '4200',
...
async function runningServices() {
  const [web, api] = await Promise.all([
    fetch('http://localhost:3200/', { signal: AbortSignal.timeout(5000) })
      .then(async (response) => response.ok && (await response.text()).includes('TripDock'))
      .catch(() => false),
    fetch('http://127.0.0.1:4200/graphql', {
      method: 'POST',
...
try {
  ...
  if (existing.web && existing.api) {
    console.log('\nTripDock is already running. Open http://localhost:3200\n');
    process.exit(0);
  }
  console.log('\nStarting the packing preview. Your main app stays separate.\n');
  try {
  ...
  await run('docker', ['compose', '-f', 'compose.packing.yaml', 'up', '-d', '--wait']);
  ...
  if (!stopping) {
    ...
    if (!running.web) services.push(['--filter', '@tripdock/web', 'dev', '--port', '3200', '--strictPort']);
    ...
    console.log('\nOpen http://localhost:3200 when the web server is ready. Existing services are reused.\n');
    if (services.length) console.log('Press Ctrl+C to stop services started by this command.\n');
  }
} catch (error) {
  console.error(`\nCould not start the preview: ${error.message}\nMake sure Docker Desktop is running, then run pnpm dev:packing again.`);
  await stop(1);
}
